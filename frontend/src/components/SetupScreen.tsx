...
export function SetupScreen({ onReady }: { onReady: () => void }) {
  ...
  const configureGroq = async () => {
    ...
    setSavingKey(true);
    try {
      await saveCloudKey('GROQ_API_KEY', groqKey.trim());
    ...
    >
      <div className="w-full max-w-md px-6">
        ...
        <div className="flex flex-col gap-2 mb-8">
          {(status?.source === 'groq'
            ? [
                { key: 'ollama_ready' as const, label: 'Groq', icon: Cpu, detail: 'Loading secure credentials...' },
                { key: 'model_ready' as const, label: 'GPT-OSS 120B', icon: Database, detail: 'Configuring model...' },
                { key: 'server_ready' as const, label: 'Jarvis Core', icon: Server, detail: 'Starting assistant...' },
              ]
        ...
        {status?.source === 'groq' && status?.error && (
          <div className="mt-4 flex gap-2">
            <input
              type="password"
              value={groqKey}
              onChange={(event) => setGroqKey(event.target.value)}
              onKeyDown={(event) => { if (event.key === 'Enter') void configureGroq(); }}
              placeholder="gsk_..."
