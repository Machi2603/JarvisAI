...
export function InputArea() {
  ...
  const sendMessage = useCallback(async (voiceCommand?: string) => {
    const content = (voiceCommand ?? input).trim();
    if (!content) return;
    if (/^(?:jarvis[,.\s]+)?(?:para|detente|c[aá]llate|deja de hablar|silencio)\b/i.test(content)) {
      window.dispatchEvent(new Event('jarvis-stop-speaking'));
      setInput('');
      return;
    }
    if (streamState.isStreaming) return;
    const cameraIntent = await detectIntent(content).catch(() => null);
    if (cameraIntent?.intent === 'open_camera' || isCameraCommand(content)) {
      setInput('');
      window.dispatchEvent(new Event('jarvis-camera-open'));
      return;
    }
    if (cameraIntent?.intent === 'close_camera') {
      setInput('');
  ...
  useEffect(() => {
    const onJarvisCommand = (event: Event) => {
      const command = (event as CustomEvent<string>).detail;
      if (command) void sendMessage(command);
    };
    window.addEventListener('jarvis-command', onJarvisCommand);
    return () => window.removeEventListener('jarvis-command', onJarvisCommand);
  }, [sendMessage]);
  ...
  return (
    <div className="px-4 pb-4 pt-2" style={{ maxWidth: 'var(--chat-max-width)', margin: '0 auto', width: '100%' }}>
      ...
      <div className="flex items-center gap-2 border-t border-cyan-300/15 px-1 py-3 transition-colors focus-within:border-cyan-300/40">
        <textarea
          ref={textareaRef}
          value={input}
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              style={{
                ...
                color: input.trim() ? 'white' : 'rgba(103, 232, 249, 0.35)',
              }}
            >
              <Send size={16} />
            </button>
