...
export function SettingsPage() {
  ...
  const [srcKind, setSrcKind] = useState<InferenceSource['provider']>('groq');
  const [cloudModel, setCloudModel] = useState('');
  const [candidateKey, setCandidateKey] = useState('');
  const [catalogModels, setCatalogModels] = useState<Array<{ id: string; name?: string | null }>>([]);
  ...
  useEffect(() => {
    getInferenceSource().then((s) => {
      setSrcKind(s.provider || s.kind || 'groq');
  ...
  const refreshCatalog = useCallback(async () => {
    if (!desktopKeyStorage || !['groq', 'openai', 'anthropic', 'gemini'].includes(srcKind)) return;
    setCatalogLoading(true);
    try {
      const models = await listProviderModels(srcKind as 'groq' | 'openai' | 'anthropic' | 'gemini', candidateKey || undefined);
      setCatalogModels(models);
      if (!cloudModel && models[0]) setCloudModel(models[0].id);
      setSrcMsg(`Loaded ${models.length} models.`);
    } catch (e: any) {
      setCatalogModels([]);
      setSrcMsg(e?.message || 'Could not load models.');
    } finally {
      setCatalogLoading(false);
    }
  }, [candidateKey, cloudModel, desktopKeyStorage, srcKind]);

  const saveSource = useCallback(async () => {
    try {
      if (['groq', 'openai', 'anthropic', 'gemini'].includes(srcKind)) {
        await applyInferenceConfig(srcKind as 'groq' | 'openai' | 'anthropic' | 'gemini', cloudModel, candidateKey || undefined);
        setSrcMsg('Saved and Jarvis is restarting.');
      } else if (srcKind === 'custom') {
        await setInferenceSource({ provider: 'custom', kind: 'custom', host: customHost, model: customModel, engine: customEngine, apiKey: customKey || undefined });
      } else if (srcKind === 'groq') {
        await setInferenceSource({ provider: 'groq', kind: 'groq' });
      } else {
        await setInferenceSource({ provider: 'ollama', kind: 'ollama' });
      }
      if (['groq', 'openai', 'anthropic', 'gemini'].includes(srcKind)) setSrcMsg('Saved and Jarvis is restarting.');
      setSrcMsg('Saved — restart the app to apply.');
    } catch (e: any) {
      setSrcMsg(e?.message ?? 'Failed to save.');
    }
  }, [candidateKey, cloudModel, customHost, customModel, customEngine, srcKind]);
        ...
        <div className="flex flex-col gap-4">
          ...
          <Section title="Inference source">
            <SettingRow label="Source" description="Where the app runs models. Applies after restart.">
              <select
                value={srcKind}
                onChange={(e) => { setSrcKind(e.target.value as InferenceSource['provider']); setSrcMsg(''); setCatalogModels([]); }}
                className="text-sm px-3 py-1.5 rounded-lg outline-none w-56"
                style={{ background: 'var(--color-bg-secondary)', color: 'var(--color-text)', border: '1px solid var(--color-border)' }}
              >
                <option value="groq">Groq · GPT-OSS 120B (recommended)</option>
                <option value="openai">OpenAI</option>
                <option value="anthropic">Anthropic</option>
                <option value="gemini">Gemini</option>
                {!desktopKeyStorage && <option value="ollama">Local Ollama</option>}
                {!desktopKeyStorage && <option value="custom">Custom OpenAI-compatible server</option>}
              </select>
            </SettingRow>
            {['groq', 'openai', 'anthropic', 'gemini'].includes(srcKind) && (
              <>
                <SettingRow label="API key" description="Leave blank to use the key already stored in Windows Credential Manager.">
                  <input
                    type="password"
                    value={candidateKey}
                    onChange={(e) => { setCandidateKey(e.target.value); setSrcMsg(''); }}
                    placeholder="Enter a new key to validate"
                    autoComplete="off"
                    className="text-sm px-3 py-1.5 rounded-lg outline-none w-56"
                    style={{ background: 'var(--color-bg-secondary)', color: 'var(--color-text)', border: '1px solid var(--color-border)' }}
                  />
                </SettingRow>
                <SettingRow label="Model" description="Choose a catalog model or enter any supported model ID.">
                  <div className="flex items-center gap-2">
                    <input
                      list="provider-models"
                      type="text"
                      value={cloudModel}
                      onChange={(e) => { setCloudModel(e.target.value); setSrcMsg(''); }}
                      placeholder="Model identifier"
                      className="text-sm px-3 py-1.5 rounded-lg outline-none w-56"
                      style={{ background: 'var(--color-bg-secondary)', color: 'var(--color-text)', border: '1px solid var(--color-border)' }}
                    />
                    <datalist id="provider-models">
                      {catalogModels.map((model) => <option key={model.id} value={model.id}>{model.name || model.id}</option>)}
                    </datalist>
                    <button onClick={() => void refreshCatalog()} disabled={catalogLoading} className="p-1.5 rounded cursor-pointer" title="Refresh model catalog">
          ...
          <Section title="Models">
            ...
            <SettingRow label="Cloud providers" description="Green dot means API key is configured">
              <div className="flex flex-wrap gap-3">
                <CloudProviderStatus label="Groq" keyName="GROQ_API_KEY" />
          ...
          <Section title="API Keys">
            <SettingRow label="Groq" description="Jarvis primary language model">
              <ApiKeyInput keyName="GROQ_API_KEY" placeholder="gsk_..." />
            </SettingRow>
