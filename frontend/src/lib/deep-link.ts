/**
 * deep-link.ts — Handler for jarvisai:// deep links in the Tauri desktop app.
 *
 ...
 *   jarvisai://research/{session_id}   → open a research session
 *   jarvisai://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse a `jarvisai://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `jarvisai://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `jarvisai:` scheme.
 *
 * @example
 * parseDeepLink("jarvisai://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("jarvisai://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "jarvisai:") return null;

    // The URL constructor treats "jarvisai://research/abc123" such that
