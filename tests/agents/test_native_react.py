...
class _TimeStub(BaseTool):
    tool_id = "current_time"

    @property
    def spec(self) -> ToolSpec:
        return ToolSpec(name="current_time", description="Current time.")

    def execute(self, **params) -> ToolResult:
        return ToolResult(tool_name="current_time", content="12:34", success=True)
...
def test_time_question_uses_local_time_tool_without_model_guessing():
    engine = MagicMock()
    engine.engine_id = "mock"
    result = NativeReActAgent(engine, "test-model", tools=[_TimeStub()]).run(
        "Que hora es ahora?"
    )

    assert result.content == "12:34"
