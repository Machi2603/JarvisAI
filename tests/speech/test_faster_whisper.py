...
def test_faster_whisper_closes_and_removes_uploaded_audio_before_reading():
    """Windows decoders must be able to reopen the temporary WebM file."""
    mock_model = MagicMock()
    mock_info = MagicMock(language="es", language_probability=0.9, duration=0.0)
    captured: dict[str, object] = {}

    def read_audio(path: str, **_kwargs):
        captured["path"] = path
        captured["audio"] = Path(path).read_bytes()
        return iter(()), mock_info

    mock_model.transcribe.side_effect = read_audio
    backend = FasterWhisperBackend(device="cpu")
    backend._model = mock_model

    backend.transcribe(b"webm audio", format="webm")

    assert captured["audio"] == b"webm audio"
