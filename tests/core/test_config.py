...
class TestAgentConfigNew:
    ...
    def test_default_system_prompt_anchors_identity(self) -> None:
        """#540: the wording must name Jarvis and explicitly deny the model's
        training identity so distilled models stop claiming to be
        Claude/ChatGPT/etc. Rebranded to a Spanish 'Eres Jarvis' persona."""
        prompt = AgentConfig().default_system_prompt
        assert "Jarvis" in prompt
        assert "OpenJarvis" not in prompt
        assert "Claude" in prompt and "ChatGPT" in prompt
