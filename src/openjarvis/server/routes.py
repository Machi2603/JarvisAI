...
async def _handle_stream_agent(
    ...
    *,
    ...
    """Run the tool agent, then return its completed answer as SSE."""
    response = await asyncio.to_thread(
        _handle_agent,
        agent,
        model,
...
async def list_models(request: Request) -> ModelListResponse:
    ...
    deployed_model = os.environ.get("OPENJARVIS_LITELLM_MODEL")
    if deployed_model:
        return ModelListResponse(data=[ModelObject(id=deployed_model)])
