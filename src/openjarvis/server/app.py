...
from openjarvis.server.browser_routes import router as browser_router
...
def create_app(
    ...
    *,
    ...
    app.state.jarvis_detector = None
    model_path = pathlib.Path("training/jarvis/model/jarvis_detector.joblib")
    if model_path.exists():
        try:
            from openjarvis.speech.jarvis_detector import JarvisDetector

            app.state.jarvis_detector = JarvisDetector(model_path)
            logger.info("Personal Jarvis wake-word model loaded")
        except Exception as exc:
            logger.warning("Personal Jarvis wake-word model unavailable: %s", exc)
