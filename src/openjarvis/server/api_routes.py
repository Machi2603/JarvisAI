...
intent_router = APIRouter(prefix="/v1/intents", tags=["intents"])


@intent_router.post("/detect")
async def detect_intent(req: IntentRequest):
    from openjarvis.intents import detect_intent as classify

    intent, confidence = await asyncio.to_thread(classify, req.text)
    return {"intent": intent, "confidence": confidence}


@speech_router.websocket("/wake-word")
async def wake_word_stream(websocket: WebSocket):
    """Detect the personal Jarvis wake word from 16 kHz mono PCM frames."""
    from openjarvis.server.auth_middleware import websocket_authorized

    expected_key = getattr(websocket.app.state, "api_key", "")
    detector = getattr(websocket.app.state, "jarvis_detector", None)
    if not websocket_authorized(websocket, expected_key) or detector is None:
        await websocket.close(code=1008)
        return
    await websocket.accept()
    import numpy as np

    from openjarvis.speech.jarvis_detector import (
        WAKE_CONFIRMATIONS,
        update_wake_streak,
    )
    ...
    try:
        while True:
            ...
            samples = np.concatenate((samples, frame))[-16_000:]
            now = time.monotonic()
            if len(samples) != 16_000 or now - last_detected <= 2:
                continue
            wake_streak = update_wake_streak(detector.score(samples), wake_streak)
            if wake_streak >= WAKE_CONFIRMATIONS:
                last_detected = now
                wake_streak = 0
                samples = np.empty(0, dtype=np.float32)
                await websocket.send_json({"type": "detected"})
...
async def synthesize_speech(req: SynthesizeRequest, request: Request):
    """Synthesize text to speech and return the audio (WAV) for playback.

    Jarvis' voice: the web client plays this WAV and keeps the orb in its
    'speaking' state while it runs, so the sphere reacts to the spoken output.
    """
    ...
    voice_id = req.voice_id or getattr(speech_cfg, "voice_id", "") or "em_alex"
    speed = (
        req.speed if req.speed is not None else getattr(speech_cfg, "voice_speed", 1.0)
    )
    ...
    return Response(
        ...
        headers={
            "X-Voice-Id": str(result.voice_id or voice_id),
            "X-Audio-Duration": str(result.duration_seconds or 0.0),
        },
    )
