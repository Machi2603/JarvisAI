...
class NativeReActAgent(ToolUsingAgent):
    ...
    def _obvious_tool_call(self, text: str) -> ToolCall | None:
        """Use reliable local tools before asking a small model to plan."""
        url = re.search(r"https?://[^\s]+", text)
        names = {tool.spec.name for tool in self._tools}
        if url and "browser_navigate" in names:
            return ToolCall(
                id="browser_url",
                name="browser_navigate",
                arguments=json.dumps({"url": url.group(0).rstrip(".,;)")}),
            )
        if (
            re.search(r"\b(hora|fecha|time|date)\b", text, re.IGNORECASE)
            and "current_time" in names
        ):
            return ToolCall(id="current_time", name="current_time", arguments="{}")
        if (
            re.search(
                r"\b(busca|buscar|search|investiga|internet|web)\b",
                text,
                re.IGNORECASE,
            )
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        obvious_call = self._obvious_tool_call(input)
        if obvious_call is not None:
            tool_result = self._executor.execute(obvious_call)
            if obvious_call.name == "browser_navigate" and tool_result.success:
                url = tool_result.metadata.get("url", "la página")
                content = f"He abierto {url} en Jarvis Browser."
            else:
                content = tool_result.content
            self._emit_turn_end(turns=1)
            return AgentResult(content=content, tool_results=[tool_result], turns=1)
