...
def serve(
        ...
        try:
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    ...
                    configured = (
                        os.environ.get("OPENJARVIS_AGENT_TOOLS") or config.agent.tools
                    )
                    if configured:
                    ...
                    for name in ToolRegistry.keys():
                        ...
                        if isinstance(tool_cls, type) and issubclass(
                            ...
                            if name == "file_read" and os.environ.get(
                                "OPENJARVIS_WORKSPACE"
                            ):
                                tools.append(
                                    tool_cls(
                                        allowed_dirs=[
                                            os.environ["OPENJARVIS_WORKSPACE"]
                                        ]
                                    )
                                )
                            else:
    ...
    # Set up text-to-speech backend (Jarvis' voice)
    tts_backend = None
    try:
        ...
        if tts_backend:
            console.print(
                f"  Voice: [cyan]{tts_backend.backend_id}[/cyan] "
                f"({config.speech.voice_id})"
            )
    except Exception as exc:
