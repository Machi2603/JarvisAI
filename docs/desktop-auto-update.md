...
The Jarvis desktop app ships with [Tauri's updater
...
on launch / every 30 min
        │
        ▼
GET https://github.com/<current-repository>/releases/download/desktop-latest/latest.json
        │
        ▼
...
The release workflow overrides the updater endpoint with the active `${{ github.repository }}`, so forks publish updates from their own repository rather than upstream.

## Verifying a release manually

```bash
# Download the latest manifest and confirm it parses cleanly
curl -fsSL https://github.com/Machi2603/JarvisAI/releases/download/desktop-latest/latest.json | jq .
